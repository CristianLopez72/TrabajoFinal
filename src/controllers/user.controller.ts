...
export const signUp = (req: Request, res: Response, next: NextFunction) =>{
    passport.authenticate('signup',{failureMessage: badRequestMessage}, async (error:any, user:any, info:any) =>{
        if(error){
            logger.error(error)
            return next(error)
        }
...
export const login = (req:Request ,res:Response ,next:NextFunction) =>{
    passport.authenticate('login', {failureMessage: badRequestMessage}, (error:any, user:any, info:any) =>{
        if(error){
            logger.error(error)
            return next(error)
        }
